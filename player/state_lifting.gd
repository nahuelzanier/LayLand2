...
func _process(_delta):
	...
	if vertical || horizontal:
		...
		state_manager.change_state("lift_walking", direction)
...
func play_animation(h, v):
	...
	if angle<=7*PI/8 && angle>=5*PI/8:
		state_manager.animations.animation = "lift_north"
	elif angle<=-PI/8 && angle>=-3*PI/8:
		state_manager.animations.animation = "lift_south"
	elif angle>=-7*PI/8 && angle<=-5*PI/8:
		state_manager.animations.animation = "lift_west"
	elif angle>=PI/8 && angle<=3*PI/8:
		state_manager.animations.animation = "lift_east"
	elif angle>7*PI/8 || angle<-7*PI/8:
		state_manager.animations.animation = "lift_north_west"
	elif angle>-5*PI/8 && angle<-3*PI/8:
		state_manager.animations.animation = "lift_south_west"
	elif angle<5*PI/8 && angle>3*PI/8: 
		state_manager.animations.animation = "lift_north_east"
	elif angle<PI/8 && angle>-PI/8:
		state_manager.animations.animation = "lift_south_east"
